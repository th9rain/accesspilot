...
1. 把 `src/App.tsx` 里的静态 `scenarios` 换成真实屏幕观察服务；
2. 端侧模型生成 `ScreenObservation`；
3. 云侧 Gemma planner 根据 observation 生成 `NextAction`；
4. Safety guard 对 `NextAction` 生成 `GuardDecision`；
5. 只有 `allow` 的动作进入 OpenClaw 工具网关；
6. 高风险动作进入确认流程；
7. 状态存储只保存脱敏摘要，不保存原始敏感截图。
